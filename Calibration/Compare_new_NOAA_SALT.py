...
obs = loadz("/home/bootk/git_liu/APS/Obs/NDBC/NDBC_NC_ocean.npz")
...
mod = loadz("/home/bootk/git_liu/APS/Calibration/mod_at_new_noaa_stations.npz")
...
#41024 ILM3 - Wrightsville Beach Offshore
#41037 SUN2 - Sunset Nearshore
...
for st,sta in enumerate(stations): 
    ...
    plot(mod.time + datenum(2019, 1, 1) + 00 / 24, myi,"b-") # plot model time (x) every 24 hours (??) with filtered elevation (y) as blue line
    setp(gca(),xticks=xts, xticklabels=xls, xlim=[datenum(2019, 1, 1), datenum(2019, 12, 31)],ylim=[0, 37])
