...
for si=1:4 % (si-1)*3+1:(si-1)*3+3
    mag_si=[];
    dir_si=[];
    for yr=1:length(yrs)
    oa=mag((si-1)*3+1+12*(yr-1):(si-1)*3+3+12*(yr-1));
    mag_si =cat(1,mag_si,oa);

    oa=dir((si-1)*3+1+12*(yr-1):(si-1)*3+3+12*(yr-1));
    dir_si =cat(1,dir_si,oa);
    end
mag_seas(:,si)=mag_si;
dir_seas(:,si)=dir_si;
