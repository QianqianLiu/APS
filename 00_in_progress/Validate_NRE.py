...
def calculate_rmse(observed, modeled):
    return np.sqrt(mean_squared_error(observed, modeled))

# Function to calculate R-squared
def calculate_r_squared(observed, modeled):
    return r2_score(observed, modeled)
...
for i,sta in enumerate([20, 30, 50, 70,100,160]):#enumerate(stations[1:19:3]): #arange(0,190,10):
    ...
    r2 = calculate_r_squared(S.temp[pd], mod.temp[pdm,:])
    rmse = calculate_rmse(S.temp[pd], mod.temp[pdm,:])
    
    text(datenum(2019, 1, 1), 28, f'R² = {r2:.2f}\nRMSE = {rmse:.2f}', fontsize=10, bbox=dict(facecolor='white', alpha=0.5))
