...
#input
#------------------------------------------------------------------------------
StartT=datenum(2019,1,1)
grd='grid.npz'
dir_hycom='/home/liuq/Analysis/hycom_2019/Data/'
...
subplot(2,2,1)
gd.plot(fmt=1,value=tr_nd[:,0,0]) # bottom temperature
title('Bottom Temperature')

subplot(2,2,2)
gd.plot(fmt=1,value=tr_nd[:,31,0]) # surface temperature
title('Surface Temperature')


subplot(2,2,3)
gd.plot(fmt=1,value=tr_nd[:,0,1]) # bottom temperature
title('Bottom Salinity')

subplot(2,2,4)
gd.plot(fmt=1,value=tr_nd[:,31,1]) # surface temperature
title('Surface Salinity')

#gcf().tight_layout()
savefig('./input/Hostart_TS',dpi=200)
